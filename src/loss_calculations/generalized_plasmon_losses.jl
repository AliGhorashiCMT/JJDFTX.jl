...
function first_order_damping(HWannier::Array{Float64, 3}, cell_map::Array{Float64, 2}, HePhWannier::Array{<:Real, 5}, cellMapEph::Array{<:Real, 2},
    ...
    for (xmesh, ymesh) in Tuple.(CartesianIndices(rand(mesh, mesh)))
        ...
        x = xmesh-mesh/2
        y = ymesh-mesh/2
        kinitial = [x/(subsampling*mesh), y/(subsampling*mesh), 0] + offset
            ...
            for (g1, g2, ϵphonon) in zip(eachslice(phonon_mat_elements1; dims=1), eachslice(phonon_mat_elements2; dims=1), phonon_energies)
                ...
                for (findex, (ϵfinal, ffinal)) in enumerate(zip(ϵfinals, ffinals))
                    ...
                    for (iindex, (ϵinitial, finitial)) in enumerate(zip(ϵinitials, finitials))
                        ...
                        (ω > 0) && (lossarray[round(Int, ω*histogram_length+1)] += 1/cell_area*(abs(T)^2)*2π/ħ*e²ϵ/4*ω/qabs*finitial*ffinal*(1/mesh)^4*histogram_length*(1/subsampling^4))
                    end
                end
            end
        end
