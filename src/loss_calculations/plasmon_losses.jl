...
function landau_damping(wannier_file::AbstractString, cell_map_file::AbstractString, lattice_vectors::Vector{<:Vector{<:Real}}, histogram_width::Integer, mesh::Integer, q::Vector{<:Real}, μ::Real, energy_range::Real) 
    ...
    for (xmesh, ymesh) in Tuple.(CartesianIndices(rand(mesh, mesh)))
        ...
        f2 = ϵ2>μ ? 1 : 0
        ω = ϵ2-ϵ1
        (ω > 0) && (lossarray[round(Int, (ω+offset)*histogram_length  )] += 2π/ħ*e²ϵ/4*ω/qabs*f1*f2*(1/mesh)^2*histogram_width)
    end
    return lossarray
end
...
function first_order_damping(HWannier::Array{Float64, 3}, cell_map::Array{Float64, 2}, lattice_vectors::Vector{<:Vector{<:Real}}, q::Vector{<:Real}, μ::Real, 
    ...
    for (xmesh, ymesh) in Tuple.(CartesianIndices(rand(mesh, mesh)))
        ...
        for (xmesh1, ymesh1) in Tuple.(CartesianIndices(rand(mesh, mesh)))
            ...
            ffinal = ϵfinal>μ ? 1 : 0
            ω = ϵfinal-ϵinitial+ϵphonon
            (ω > 0) && (lossarray[round(Int, ω*histogram_width + 1 )] += 1/cell_area*(fmiddle1/(ϵmiddle-ϵinitial-ω)+ fmiddle2/(ϵmiddle2-ϵinitial+ϵphonon))^2*gph^2*2π/ħ*e²ϵ/4*ω/qabs*finitial*ffinal*(1/mesh)^4*histogram_width)
        end
    end
    return lossarray
end
...
$(TYPEDSIGNATURES)
"""
function second_order_damping(wannier_file::String, cell_map_file::String, lattice_vectors::Array{<:Array{<:Real, 1}, 1}, q::Array{<:Real, 1}, μ::Real, ϵphonon::Real, gph::Real; histogram_width::Real=100, mesh::Int=30, energy_range::Real=10) 
    ...
    for (xmesh, ymesh) in Tuple.(CartesianIndices(rand(mesh, mesh)))
        ...
        for (xmesh1, ymesh1) in Tuple.(CartesianIndices(rand(mesh, mesh)))
            ...
            for (xmesh2, ymesh2) in Tuple.(CartesianIndices(rand(mesh, mesh)))
                ...
                ffinal = ϵfinal>μ ? 1 : 0
                ω = ϵfinal-ϵinitial+ϵphonon
                ω > 0 && (lossarray[round(Int, ω*histogram_length + 1 )] += 1/cell_area*( 1/(ϵmiddle-ϵinitial-ω)*1/(ϵsecondmiddle2-ϵinitial-ω+ϵphonon)*fmiddle1*fsecondmiddle2 + fmiddle2/(ϵmiddle2-ϵinitial+ϵphonon)*( fsecondmiddle1/(ϵsecondmiddle1-ϵinitial+2*ϵphonon) + fsecondmiddle2/(ϵsecondmiddle2-ϵinitial-ω+ϵphonon) ))^2*gph^2*2π/ħ*e²ϵ/4*ω/qabs*finitial*ffinal*(1/mesh)^6*histogram_width)
            end
        end
    end
    return lossarray
