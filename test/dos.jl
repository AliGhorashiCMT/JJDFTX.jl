...
@testset "Check Histogramming Compatibility with Numerical Integration" begin
    ...
    wannier_vectors(Hwannier, cell_map, rand(3)) ≈ [1] #Check that one band model indeed has no nontrivial band eigenvectors
    wannier_vectors("wannier.txt", "wannier.map.txt", rand(3), 1) ≈ [1] #Check that one band model indeed has no nontrivial band eigenvectors
end